...
from preprocess.visual_cloze_minibatching import *
...
def validate(fold_name, fold_data, fold_file, val_batch_size=1024):
    ...
    for start, end in batches:
        for batch in generate_minibatches_from_megabatch(fold_data, start, end, 
            difficulty=args.difficulty, fold_dict=read_fold(fold_file), 
            shuffle_candidates=True):

            prods = pred_fn(*batch[1:-2])
            labels = np.argmax(batch[-2], axis=-1)
...
if __name__ == '__main__':
    ...
    parser = argparse.ArgumentParser(description='visual cloze models')
    parser.add_argument('-data', default='data/comics.h5')
    parser.add_argument('-vocab', default='data/comics_vocab.p')
    parser.add_argument('-model', default='image_text', 
    ...
    print 'training %s model for %s visual_cloze with d_word=%d, d_hidden=%d' %\
        (args.model, args.difficulty, args.d_word, args.d_hidden)
    log = open('logs/%s_%s_%s_%ddword_%ddhidden.log' % (args.model, 'visual_cloze', \
    ...
    log = open('logs/%s_%s_%s_%ddword_%ddhidden.log' % (args.model, 'visual_cloze', \
        args.difficulty, args.d_word, args.d_hidden), 'w')
    dev_fold = 'folds/%s_%s_%s.csv' % ('visual_cloze', 'dev', args.difficulty)
    test_fold = 'folds/%s_%s_%s.csv' % ('visual_cloze', 'test', args.difficulty)
    ...
    for epoch in range(args.n_epochs):
        ...
        for start, end in train_batches:
            for batch in generate_minibatches_from_megabatch(train_data, 
                start, end, context_size=3, difficulty=args.difficulty, 
                shuffle_candidates=True):

                batch_loss = train_fn(*batch[1:-1])
